...
void displayNumber(s16 number)
{
    ...
    /* This part of code is for display the negative sign, since there is only 4
     * segment, the maximum number that can displayed is -999.
     * Therefore, check if the number is less than or equal to -999, display
     * dashes in all four segements
     */
	else if(number>=-999 && number<0){
            //show the absolute value of negative number
		    absNum=-number;
		    calculateDigits(absNum);
            //display dash as negative sign
